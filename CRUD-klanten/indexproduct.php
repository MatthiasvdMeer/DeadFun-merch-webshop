...
.hero {
    ...
    background: url('images/yt logo.png') center/cover no-repeat;
    display: flex;
...
<table>
    <thead>
        <tr>
            ...
            <th>Naam</th>
            <th>Prijs</th>
            <th>Foto</th> <!-- New column for the image -->
    ...
    <tbody>
        ...
        $records = getAllRecords(); // Assuming this function fetches all products
        if (!empty($records)) {
            foreach ($records as $row) {
                echo "<tr>";
                echo "<td>{$row['prodID']}</td>";
                echo "<td>{$row['prodNaam']}</td>";
                echo "<td>{$row['prijs']}</td>";
                echo "<td><img src='data:image/jpeg;base64," . base64_encode($row['Foto']) . "' alt='Product Image' style='width:100px;height:auto;'></td>"; // Display the image
                echo "<td>{$row['leverancierID']}</td>";
                echo "<td><a href='updateproduct.php?id={$row['prodID']}'>Wijzig</a></td>";
                echo "</tr>";
            }
        } else {
